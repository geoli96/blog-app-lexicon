...
export default async function Home({ searchParams }: { searchParams: Promise<{ search?: string; category?: string; page?: string; searchFilter?:string }> }) {
...
function PostCard({ post, featured }: { post: Post; featured: boolean }) {
  return <Link className={`${styles.postCard} ${featured ? styles.featured : ""}`} href={`/posts/${post.id}`}>
    ...
    <div className={styles.cardContent}><div className={styles.cardMeta}><span>{post.category}</span><span>{post.date}</span></div><h3>{post.title}</h3><p>{post.excerpt}</p><span  className={styles.author}>Written by {post.createdBy} </span><span className={styles.readLink}>Read post </span></div>
  </Link>;
}
